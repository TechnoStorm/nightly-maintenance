...
if [[ ! -f "$LOG_FILE" ]]; then
    ...
    echo "[ERROR]: Не удалось создать лог-файл"
    exit 1
    fi
...
[[ -f "$SYNC_DIR" ]]                          || fail "Директория сихронизации Syncthing не найдена: $SYNC_DIR"
[[ -f "$SYNC_BACKUP_DIR" ]]                   || fail "Директория для зеркалирования папок Syncthing не найдена: $SYNC_BACKUP_DIR"
