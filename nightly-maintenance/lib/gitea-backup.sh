...
log "Запущен сценарий резервного копирования данных Gitea"
...
while systemctl is-active --quiet gitea; do
    ...
    if (( timer >= MAX_WAIT )); then
        ...
        log "Запущен сценарий резервного копирования данных Gitea прерван"
        exit 1
    fi
done
...
log "Сценарий резервного копирования данных Gitea успешно завершён"
